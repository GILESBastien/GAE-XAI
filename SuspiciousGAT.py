...
class Both(BaseDetector):
	...
	def fit(self, G, y_true=None):
		...
		#print(G.edge_index)
		#temp=to_dense_adj(G.edge_index)[0]
		#print(temp)
		#G.s = temp
		...
		for epoch in range(self.epoch):
			...
			for sampled_data in loader:
				...
				score,at,adj = self.loss_func(G.x[:batch_size],
									   x_[:batch_size],
									   G.s[:batch_size, node_idx],
									   s_[:batch_size])
...
#G.s = to_dense_adj(G.edge_index)[0]
		...
		for sampled_data in loader:
			...
			#x, s, edge_index = self.process_graph(sampled_data)
			torch.cuda.empty_cache() 
			x_, s_ = self.model(G.x, G.edge_index)
			score,at,adj = self.loss_func(G.x[:batch_size],
								   x_[:batch_size],
								   G.s[:batch_size, node_idx],
								   s_[:batch_size])
			...
			outlier_scores[node_idx[:batch_size]] = score.detach() \
				.cpu().numpy()
		return outlier_scores,torch.pow(G.x-x_,2),torch.pow(G.s - s_,2),at,adj
...
# attribute reconstruction loss
		...
		structure_errors = (torch.sum(diff_structure, 1))
