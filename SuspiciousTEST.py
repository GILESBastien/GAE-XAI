...
from gcn_conv import GCNConv as newConv
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpointGCN.pt', trace_func=print):
        """
        Args:
...
class Both(BaseDetector):
	...
	def fit(self, G, y_true=None):
		...
		for epoch in range(self.epoch):
			...
			for sampled_data in loader:
				...
				x_,s_,sa1,sa2 = self.model(x, edge_index)
				score,at,ad= self.loss_func(x[:batch_size],
									   x_[:batch_size],
			...
			train_losses.append(loss_train.detach().item())
			# val_losses.append(loss_train.detach().item())
	...
	def decision_function(self, G):
		...
		self.model.load_state_dict(torch.load('checkpointGCN.pt'))
		self.model.eval()
		...
		for sampled_data in loader:
			...
			x_, s_,sa1,sa2 = self.model(x, edge_index)
			score,at,adj = self.loss_func(x[:batch_size],
								   x_[:batch_size],
								   s[:batch_size, node_idx],
								   s_[:batch_size])
			...
			outlier_scores[node_idx[:batch_size]] = score.detach() \
				.cpu().numpy()
		return outlier_scores,torch.pow(x-x_,2),torch.pow(s-s_,2),at,adj,sa1,sa2
	...
	def loss_func(self, x, x_, s, s_):
		...
		# attribute_errors = torch.sum(torch.absolute(x-x_),1)
		# structure reconstruction loss
		# diff_structure = torch.pow(s - s_, 2)
		# structure_errors = torch.sqrt(torch.sum(diff_structure, 1))
		# structure_errors = torch.sum(torch.absolute(s-s_),1)
		
		# attribute reconstruction loss
		diff_attribute = torch.pow(x - x_, 2)
		attribute_errors = (torch.sum(diff_attribute, 1))
		# attribute_errors = torch.sum(torch.absolute(x-x_),1)
		# structure reconstruction loss
		#diff_structure = torch.pow(s - s_, 2)
		structure_errors = (torch.sum(s_, 1))
		# structure_errors = torch.sum(torch.absolute(s-s_),1)
		...
		return score,attribute_errors,structure_errors
...
class Both_Base(nn.Module):
	def __init__(self,
		...
		self.shared_encoder = GATv2Conv(in_channels=in_dim,
								  out_channels=hid_dim,
								  dropout=dropout)
		#self.shared_encoder2 = GATv2Conv(in_channels=hid_dim,
							#	  out_channels=hid_dim,
								#  dropout=dropout, add_self_loops=False)

		self.attr_decoder = GATv2Conv(in_channels=hid_dim,
								out_channels=in_dim,
								dropout=dropout)

		# self.shared_encoder = SAGEConv(in_channels=in_dim,
								  # out_channels=hid_dim,
								
								  # )

		# self.attr_decoder = SAGEConv(in_channels=hid_dim,
								# out_channels=in_dim,
								# )

		# self.struct_decoder = SAGEConv(in_channels=hid_dim,
								  # out_channels=in_dim,
								  # )
		# self.shared_encoder = GCNConv(in_channels=in_dim,
								  # out_channels=hid_dim,
								
								  # )

		# self.attr_decoder = GCNConv(in_channels=hid_dim,
								# out_channels=in_dim,
								# )

		# self.struct_decoder = GCNConv(in_channels=hid_dim,
								  # out_channels=in_dim,
								  # )

	def forward(self, x, edge_index):
		# encode
		h,edge_att1 = self.shared_encoder(x, edge_index)
		#h,edge_att = self.shared_encoder(h, edge_index)
		# decode feature matrix
		x_,edge_att2 = self.attr_decoder(h, edge_index)
		# decode adjacency matrix
		h_ = self.struct_decoder(h, edge_index)

		#s_=torch.squeeze(to_dense_adj(edge_att1[0],None,edge_att1[1])[0],2)-torch.squeeze(to_dense_adj(edge_att2[0],None,edge_att2[1])[0],2)
		#print(torch.squeeze(s_,2).shape)
		s_ = h_ @ h_.T
		#print(s_.shape)
		...
		return x_,s_,edge_att1,edge_att2
